...
//						System.out.println(pointsArray.get(i));
	...
	public static int countUsingAlgorithm02(int startIndex, int endIndex, int numOfDiamensions, ArrayList<ArrayList<Float>> pointsArray) {
		...
		if (endIndex - startIndex <= 2)
			...
			countUsingAlgorithm02(startIndex, startIndex + midIndex - 1, numOfDiamensions, pointsArray);
			countUsingAlgorithm02(startIndex + midIndex, endIndex, numOfDiamensions, pointsArray);
		}
		// Count the number of interesting points
		for (int i = 0; i < endIndex; i++)
			if (carPointsArray.get(i).get(0) != -999.999f)
	...
	public static int SimpleAlgo2(int n, int d, ArrayList<ArrayList<Float>> pointsArray) {
		...
		for (int i = 0; i < n; i++)
			if (isBest.get(i))
				count++;
		/*
		 * //Printing interesting points for (int i=0; i<n; i++) if (isBest.get(i)) {
		 * for (int k=0; k<d; k++) System.out.print(pointsArray.get(i).get(k) + "");
		 * System.out.println(); }
		 */
		return count;
	} // end SimpleAlgo2
