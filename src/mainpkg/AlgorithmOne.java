...
public class AlgorithmOne {
	...
	public static ArrayList<ArrayList<Float>> readInput(String fileName, int rowsInFile, int dimensionsInFile, ArrayList<ArrayList<Float>> pointsArray) {
		...
		try {
			...
			try {
				rows = rowsInFile = inputScanner.nextInt();
				dimensions = dimensionsInFile = inputScanner.nextInt();
...
//			System.out.println("PointsArray size::" + pointsArray.size());
	...
	public static int SimpleAlgo2(int n, int d, ArrayList<ArrayList<Float>> pointsArray) {
		...
		System.out.println(n + " is the value of n");
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (isBest.get(i)) {
...
//							System.out.println("pointsArray.i.k > pointsArray.j.k::::: "+pointsArray.get(i).get(k) +" :: "+ pointsArray.get(j).get(k)); 
	...
	public static void main(String[] args) throws InterruptedException {
		ArrayList<ArrayList<Float>> carPointsArray = new ArrayList<ArrayList<Float>>(); // This holds the dimensional array
...
//		readInput(5,2); /* This can be used for small input set */
		...
		carPointsArray = readInput(filePath, rows, dimensions, carPointsArray);
		long start = System.nanoTime(); /* Record starting time */

		System.out.println("Number of interesting points using SimpleAlgo2 : "
				+ SimpleAlgo2(rows, dimensions, carPointsArray)); /* Calling SimpleAlgo2 */
